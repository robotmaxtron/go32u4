...
func (m *ATmega32u4) ReadSRAM(address uint16) uint8 {
	if address < 32 { return m.CPU.Reg[address] }
	if address < 32+uint16(len(m.IORegData)) { return m.ReadIO(address - 32) }
	sramAddr := address - (32 + uint16(len(m.IORegData)))
	if int(sramAddr) < len(m.SRAMData) { return m.SRAMData[sramAddr] }
	return 0
}

func (m *ATmega32u4) WriteSRAM(address uint16, value uint8) {
	if address < 32 { m.CPU.Reg[address] = value } else if address < 32+uint16(len(m.IORegData)) {
		m.WriteIO(address-32, value)
	} else {
		sramAddr := address - (32 + uint16(len(m.IORegData)))
		if int(sramAddr) < len(m.SRAMData) { m.SRAMData[sramAddr] = value }
	}
}

func (m *ATmega32u4) ReadIO(address uint16) uint8 {
	if address == 0x3F { return m.CPU.SREG }
	if address == 0x3E { return uint8(m.CPU.SP >> 8) }
	if address == 0x3D { return uint8(m.CPU.SP & 0xFF) }
	return m.Periph.IOCallback(address, 0, false)
}

func (m *ATmega32u4) WriteIO(address uint16, value uint8) {
	if address == 0x3F { m.CPU.SREG = value; return }
	if address == 0x3E { m.CPU.SP = (m.CPU.SP & 0x00FF) | (uint16(value) << 8); return }
	if address == 0x3D { m.CPU.SP = (m.CPU.SP & 0xFF00) | uint16(value); return }
	m.Periph.IOCallback(address, value, true)
}

func (m *ATmega32u4) Flash() []uint16 { return m.FlashData[:] }

// peripherals.System implementation
...
func (m *ATmega32u4) SaveEEPROM() error {
	if m.EEPROMFile == "" { return nil }
	return os.WriteFile(m.EEPROMFile, m.Periph.EEPROM[:], 0644)
}

func (m *ATmega32u4) PinCallback(port int8, mask uint8, value uint8) {
	if m.PinCallbackFunc != nil { m.PinCallbackFunc(port, mask, value) }
}

// bus.InterruptController implementation
